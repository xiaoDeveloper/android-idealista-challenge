...
- [X] T020 [P] [US2] Create `fragment_detail.xml` with Spanish labels, image placeholder, loading, error, retry, and accessible favorite/date controls.
- [X] T021 Implement `DetailFragment` navigation argument handling and lifecycle-aware state collection in `app/src/main/java/com/xiao/idealistachallenge/ui/detail/DetailFragment.kt`.
- [X] T022 [US2] Wire listing item selection to the detail destination and document the static endpoint behavior in the navigation graph and UI copy.
